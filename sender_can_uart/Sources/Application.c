...
void APP_Run(void) {
	...
	for (;;) {
		if (UART_RxBuff_NofElements() != 0) {
			...
			unsigned char ch = 'H';
			Frame.Data = &ch; /* Set pointer to OutData buffer */
			...
			while (UART_RxBuff_NofElements() != 0) {
				...
				Frame.Length = 1; /* Set number of bytes in data frame - 4B */

				canData.isSent = FALSE;
				Error = CAN1_SendFrame(canData.handle, 1U, &Frame); /* Sends the data frame over buffer 1 */
				if (Error != ERR_OK) {
